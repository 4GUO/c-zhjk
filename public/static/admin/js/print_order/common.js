...
(function($){
	$.fn.membershipCard = function(options){
		                ...
		                success: function(data) {
		                	if(data){
		                		...
		                		$('<dt class=\'member-id\'></dt>').append('<i class=\'sex'+data.sex+'\'></i>')
		                			.append('<a href=\''+APP_URL+'/index.php?act=member_snshome&mid='+data.id+'\' target=\'_blank\'>'+data.name+'</a>'+(data.truename != ''?'('+data.truename+')':''))
		                			.appendTo(_dl);
		                		// avatar
		                		$('<dd class=\'avatar\'><a href=\''+APP_URL+'/index.php?act=member_snshome&mid='+data.id+'\' target=\'_blank\'><img src=\''+data.avatar+'\' /></a><dd>')
		                			.appendTo(_dl);
		                		// info
		                		var _info = '';
		                		...
		                		if(data.follow != 2){
		                			...
		                			$('<div class=\'send-msg\'> <a href=\''+APP_URL+'/index.php?act=member_message&op=sendmsg&member_id='+data.id+'\' target=\'_blank\'><i></i>站内信</a> </div>').appendTo(_bottom);
		                		}
		...
		function follow(o){
			...
			$.getJSON(APP_URL+'/index.php?act=member_snsfriend&op=addfollow&callback=?&mid='+data_str.mid, function(data){
				if(data){
		...
		function onfollow(o){
			var data_str = o.parent().attr('data-param');
			eval( 'data_str = '+data_str);
			$.getJSON(APP_URL+'/index.php?act=member_snsfriend&op=delfollow&callback=?&mid='+data_str.mid, function(data){
				if(data){
		...
		function circle_function(d){
			...
			$.each(d,function(i, n){
				rs.append('<div class=\'rank-div\' title=\''+n.circle_name+'圈等级'+n.cm_level+'，经验值'+n.cm_exp+'\'><a href=\''+APP_URL+'/index.php?act=group&c_id='+n.circle_id+'\' target=\'_blank\'>'+n.circle_name+'</a><i></i><em class=\'rank-em rank-'+n.cm_level+'\'>'+n.cm_level+'</em></div>');
			});
			return rs;
		}
    ...
    $.getJSON(url, {'goods_id':goods_id, 'quantity':quantity}, function(data) {
        if (data != null) {
            if (data.state) {
                ...
                $('#rtoolbar_cartlist').load(APP_URL + '/index.php?act=cart&op=ajax_load&type=html');
            } else {
                alert(data.msg);
            }
        }
