...
DialogManager = {
	...
	'close': function(id) {
		if (typeof(__DIALOG_WRAPPER__[id]) != 'undefined' && __DIALOG_WRAPPER__[id] != null  && __DIALOG_WRAPPER__[id].close()) {
...
ScreenLocker = {
	...
	'masker': [],
	'lock': function(zIndex, id) {
		var masker = $('<div id=\'' + id + '_locker\' class=\'dialog_manage_screen_locker\'></div>');
		this.masker.push(masker);
		/* 样式 */
		masker.css(this.style);
		console.log(ii);
		if (zIndex) {
			masker.css('zIndex', zIndex + ii);
		} else {
			masker.css('zIndex', 999 + ii);
		}
...
Dialog = function(id) { /* 构造函数生成对话框代码，并加入到文档中 */
	this.id = id;
	this.init();
};
Dialog.prototype = { /* 唯一标识 */
	'id': null,
	/* 文档对象 */
	'dom': null,
	'lastPos': null,
	...
	'init': function() {
		...
		/* 初始化样式 */
		for (var i in __DIALOG_WRAPPER__) {
			ii++;
		}
		console.log(ii);
	...
	'show': function(pos, lock) {
		...
		/* 锁定屏幕 */
		if (lock == 1) ScreenLocker.lock(999, this.id);

		/* 显示对话框 */
		$(this.dom.wrapper).fadeIn('slow');
	},
	...
	'close': function(lock) {
		...
		$(this.dom.wrapper).hide(1000).remove();
	...
	'createContents': function(type, options) {
		var _html = '',
			self = this,
			status = 'complete';
		if (!options) { /* 如果只有一个参数，则认为其传递的是HTML字符串 */
		...
		case 'ajax':
			...
			$.get(options, function(data) {
				self.setContents(data); /* 使用上次定位重新定位窗口位置 */
		...
		case 'ajax_notice':
			...
			$.get(options, function(data) {
				...
				self.setContents(MsgTxt); /* 使用上次定位重新定位窗口位置 */
				self.setPosition(self.lastPos);
			}); /* 先提示正在加载 */
			_html = this.createContents('loading', {
				'text': '正在处理...'
		...
		case 'message':
			...
			_message_contents = $('<div class=\'dialog_message_contents dialog_message_' + type + '\'><i class=\'iocn\'></i>' + options.text + '</div>');
			...
			switch (type) {
				case 'notice':
					...
					var closefunc = function() {
						var wait = $$('wait');
						if(wait){
							timer = --wait.innerHTML;
						}
						if (timer <= 0) {
							if (typeof options.callback == 'function') options.callback();
			                else eval(options.callback);
							DialogManager.close(self.id);
							clearInterval(showDialogST);
						}
					};
					leftmsg = '<span id=\'wait\'>' + time + '</span> 秒后窗口关闭';
					var _ok_button = '<time class=\'countdown\'>' + '<i class=\'icon-time\'></i>' + leftmsg + '</time>';
					$(_buttons_bar).append(_ok_button);
					var showDialogST = setInterval(closefunc, 1000);
				case 'warning':
		...
		if (typeof(pos) == 'string') {
			switch (pos) {
			    case 'center':
					...
					/* left = (当前可视区的宽度 - 对话框的宽度 ) / 2 */
					left = ($(window).width() - dialog_width) / 2;

					/* top = (当前可视区的高度 - 对话框的高度 ) / 2 */
					top = ($(window).height() - dialog_height) / 2;
...
DIALOGIMGDIR = STATICURL + '/js/dialog/images/';
