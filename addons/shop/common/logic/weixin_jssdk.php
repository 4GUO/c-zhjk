...
class weixin_jssdk {
    ...
    private function jssdk_get_ticket($uniacid = 0) {
        $item = model('weixin_wechat')->field('wechat_access_token,wechat_expires_in,wechat_type,wechat_appid,wechat_appsecret,wechat_jssdk_ticket,wechat_jssdk_expires_in,wechat_id')->where(array('uniacid' => $uniacid))->order('wechat_jssdk_expires_in desc')->find();
        if (empty($item)) {
            return array();
        }
        ...
        if ($item['wechat_jssdk_ticket'] && $diff >= time()) {
            return array(
                ...
                'ticket' => $item['wechat_jssdk_ticket']
            );
        }
        $token = 0;
        ...
        if ($token == 0) {
            $this->access_token = logic('weixin_token')->get_accesstoken_fromwx($item['wechat_appid'], $item['wechat_appsecret'], $item['wechat_type'], $item['wechat_id']);
        }
    ...
    public function jssdk_get_signature($url, $uniacid) { //获取自定义分享配置信息
        $ticket = $this->jssdk_get_ticket($uniacid);
        if (empty($ticket)) {
            return array();
        } else {
            $tmpArr = array(
