...
class weixin_token {
    ...
    public function curl_get($url) {
        ...
        $res = curl_exec($ch);
        curl_close($ch);
        $encoding = mb_detect_encoding($res, array(
            'ASCII',
            'UTF-8',
            'GB2312',
            'GBK',
    ...
    public function get_access_token($item = array() , $first = false, $uniacid = 0) {
        $token = 0;
        if (empty($item)) {
			if (!$uniacid) {
				return '';
			}
            $item = model('weixin_wechat')->where(array('uniacid' => $uniacid))->field('wechat_access_token,wechat_expires_in,wechat_type,wechat_appid,wechat_appsecret,wechat_id')->order('wechat_expires_in desc')->find();
        }
        if (empty($item)) {
            return '';
        }
    ...
    public function get_accesstoken_fromwx($appid, $secret, $type, $wid) {
        if ($secret && $appid && in_array($type, array(
            '1',
            '2',
            '3'
        ))) {
            ...
            if (empty($result['errcode'])) {
                $data = array(
                    ...
                    'wechat_expires_in' => time() + intval($result['expires_in'])
                );
                model('weixin_wechat')->where(array('wechat_id' => $wid))->update($data);
                return $result['access_token'];
            } else {
                lib\logging::write('accesstoken获取失败' . var_export($result, true));
                return '';
            }
        } else {
            return '';
