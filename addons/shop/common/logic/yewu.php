...
class yewu
{
	...
	// 计算用户的复购见单次数
	public function get_fgjdjl_user_buy_nums($uid) {
		$buy_nums = model('shop_order')->where(array('uid' => $uid, 'state' => array(20, 30, 40, 50)))->total();
		return $buy_nums;
	}
	...
	public function fgjdjl_log($order_info, $fgjdtj_user_buy_nums, $content) {
		...
		$file_content = date('Y-m-d H:i:s') . ' 用户ID【' . $order_info['uid'] . '】 订单号【' . $order_info['order_sn'] . '】 复购见单数【' . $fgjdtj_user_buy_nums . '】 关键内容【' . $content . '】' . "\n";
	...
	public function deal_fugou_reward($order_info) {
		...
		if(!$buyer_info){
			// 记录日志
			$this->fgjdjl_log($order_info, 0, '用户不存在，不发放奖励');	
			return true;
		}
		
		// 获取复购见单的层数配置
		$fgjdtj_nums = config('fgjdtj_config');
		//订单状态：10:待付款（默认）;20:已支付;30:已发货;40:已收货;50已完成;2已取消 
		// 计算用户购买过多少单零售区商品 限制状态为已支付、已发货，已收获、已完成
		$user_buy_nums = $this->get_fgjdjl_user_buy_nums($order_info['uid']);
		if($user_buy_nums > $fgjdtj_nums){
			// 记录日志
			$this->fgjdjl_log($order_info, $user_buy_nums, '用户购买单数大于复购见单的层数，不发放奖励');	
			return true;
		}
		// 根据单数对应往上面找几层上级
		$parent_user_info = $this->get_fgjdjl_user_info($order_info['uid'], $user_buy_nums);
		if(!$parent_user_info){
			// 记录日志
			$this->fgjdjl_log($order_info, $user_buy_nums, '用户第' . $user_buy_nums . '层没有上级，不发放奖励');	
			return true;
		}
		...
		if($parent_user_buy_nums < $user_buy_nums){
			// 记录日志
			$this->fgjdjl_log($order_info, $user_buy_nums, '上级用户购买单数小于用户购买单数，不发放奖励');	
			return true;
		}
		// 获取当前用户的级别配置，获得奖励的额度
		...
		if(!isset($level_info['fgjdjl']) || $level_info['fgjdjl'] <= 0){
			// 记录日志
			$this->fgjdjl_log($order_info, $user_buy_nums, '上级用户级别没有配置复购见单奖励，不发放奖励');	
			return true;
		}
		...
		if($detail_info){
			// 记录日志
			$this->fgjdjl_log($order_info, $user_buy_nums, '用户已经发放过奖励，不发放奖励');	
			return true;
		}
		// 开启事务
		$model = model();
		...
		if($detail_id){
			...
			// 记录日志
			$this->fgjdjl_log($order_info, $user_buy_nums, '成功发放复购见单奖励' . $fgjdjl_moeny . '元给用户' . $parent_user_info['nickname']);
		}
		$model->commit();
		return true;
	}
	...
	// 订单退款奖励退回
	public function back_fgjdjl($order_info) {
		// 获取订单的奖励明细
		$detail_list = model('distribute_fgjdjl_record_detail')->where(array('order_sn' => $order_info['order_sn']))->select();
		...
		foreach($detail_list as $v){
			...
			model('distribute_fgjdjl_record_detail')->where(array('id' => $v['id']))->update(array('detail_status' => 20, 'detail_addtime' => date('Y-m-d H:i:s')));
		}
		$model->commit();
		return true;
	}
