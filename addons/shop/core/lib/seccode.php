...
class seccode
{
    ...
    function background()
    {
        ...
        if ($this->background) {
			if($this->backgrounds){
				for ($i = 0; $i < 3; $i++) {
					$start[$i] = $this->backgrounds[0];
					$end[$i] = $this->backgrounds[1];
        ...
        for ($i = 0; $i <= 3; $i++) {
            ...
            if (!empty($this->imcodefile) && file_exists($this->imcodefile)) {
                ...
                if ($this->width / 5 - $font[$i]['width'] > 0) {
                    $tt = mt_rand(0, intval($this->width / 5) - $font[$i]['width']);
                } else {
                    $tt = mt_rand(intval($this->width / 5) - $font[$i]['width'], 0);
                }
                $font[$i]['width'] += $tt;
