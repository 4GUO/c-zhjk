...
class process{
	...
	const MAX_USERS = 5;	//用户后台输入多少次被暂时锁定
	const MAX_SELLER = 6;
	...
	public static function parsekey($type){
		return str_replace(array('login','commit','reg','forget','admin','users','seller'), array(1,2,3,4,5,6,7), $type);
	}
	...
	public static function parsetimes($type){
		return str_replace(array('login', 'commit', 'reg', 'forget', 'admin','users','seller'), array(self::MAX_LOGIN, self::MAX_COMMIT, self::MAX_REG, self::MAX_FORGET, self::MAX_ADMIN, self::MAX_USERS, self::MAX_SELLER), $type);
	}
	/**
	 * 取得IP
	 *
...
class lock {
	...
	public function __construct(){
		$this->model = model()->table('form_lock');
	}
	...
	public function get($key, $type = ''){
		...
		if ($info && ($info['expiretime'] < TIMESTAMP)){
			...
			return isset($info['pvalue']) ? $info['pvalue'] : false;
		}
	}
