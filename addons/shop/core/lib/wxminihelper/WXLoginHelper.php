...
class WXLoginHelper
{
    ...
    public function __construct()
    {
        ...
        if (config('wxappid') && config('wxappsecret')) {
            $this->config['appid'] = config('wxappid');
			$this->config['secret'] = config('wxappsecret');
        }
    }
    public function checkLogin($code, $rawData, $signature, $encryptedData, $iv)
    {
        ...
        if ($res['code'] !== 200 || !isset($res['result']) || !isset($res['result'])) {
            return ['code' => ErrorCode::$RequestTokenFailed, 'message' => '请求Token失败1'];
        }
        $reqData = json_decode($res['result'], true);
        if (!isset($reqData['session_key'])) {
            return ['code' => ErrorCode::$RequestTokenFailed, 'message' => '请求Token失败2'];
        }
        $sessionKey = $reqData['session_key'];
		$openid = $reqData['openid'];
		//获得openid的session_key
		$key_info = model('wxapp_login')->where(array('openid' => $openid))->find();
		if(empty($key_info['item_id'])){
			model('wxapp_login')->insert(array('openid' => $openid, 'session_key' => $sessionKey));
			$signature2 = sha1(htmlspecialchars_decode($rawData) . $sessionKey);
		} else {
			$signature2 = sha1(htmlspecialchars_decode($rawData) . $key_info['session_key']);
			if($signature2 != $signature){
				$signature2 = sha1(htmlspecialchars_decode($rawData) . $sessionKey);
				model('wxapp_login')->where(array('item_id' => $key_info['item_id']))->update(array('session_key' => $sessionKey));
			} else {
				$sessionKey = $key_info['session_key'];
			}
		}
        /**
	...
	public function checkPhoneLogin($code, $encryptedData, $iv)
	{
		/**
         * 4.server调用微信提供的jsoncode2session接口获取openid, session_key, 调用失败应给予客户端反馈
         * , 微信侧返回错误则可判断为恶意请求, 可以不返回. 微信文档链接
        ...
        $errCode = $pc->decryptData($encryptedData, $iv, $data);
        if ($errCode !== 0) {
            return ['code' => ErrorCode::$EncryptDataNotMatch, 'message' => '解密信息错误，请重试'];
        }
        /**
         * 7.生成第三方3rd_session，用于第三方服务器和小程序之间做登录态校验。为了保证安全性，3rd_session应该满足：
         * a.长度足够长。建议有2^128种组合，即长度为16B
