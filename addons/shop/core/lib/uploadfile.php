...
class uploadfile
{
     ...
     * 允许上传的文件类型
     */
    private $allow_type = array('gif', 'jpg', 'jpeg', 'bmp', 'png', 'swf', 'tbi', 'pdf', 'pem');
    /**
     * 允许的最大文件大小，单位为KB
     */
     ...
     * 默认文件存放文件夹
     */
    private $default_dir = 'uploadfiles';
    /**
     * 错误信息
     */
    public $error = '';
    ...
    public function upfile($field)
    {
        ...
        if (move_uploaded_file($this->upload_file['tmp_name'], $this->save_path . '/' . $this->file_name)) {
            //产生缩略图
            if ($ifresize) {
                $resizeImage = new lib\resizeimage();
                $save_path = rtrim($this->save_path, '/');
                for ($i = 0; $i < count($thumb_width); $i++) {
            ...
            //删除原图
            if ($this->ifremove && is_file($this->save_path . '/' . $this->file_name)) {
                unlink($this->save_path . '/' . $this->file_name);
            }
            return SITE_URL . '/' . $this->save_path . '/' . $this->file_name;
        } else {
    ...
    public function setPath()
    {
        ...
        //判断文件夹是否可写
        if (!is_writable($this->default_dir)) {
    ...
    private function setFileName()
    {
        ...
        $tmp_name = sprintf('%010d', time() - 946656000) . sprintf('%03d', $t1 * 1000000) . sprintf('%04d', mt_rand(0, 9999));
     ...
     * @return string 字符串形式的返回结果
     */
    public function get_type($img_name)
    {
		if (is_file($img_name) && file_exists($img_name)) {
			$ename = getimagesize($img_name);
			$ename = explode('/', $ename['mime']);
			$ext = isset($ename[1]) ? $ename[1] : '';
		} else {
        ...
        if (isset($ext) && in_array($ext, array('jpg','jpeg','gif','png'))) {
            $type = $ext;
        } else {
            $type = 'string';
        }
