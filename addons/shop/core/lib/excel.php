...
class excel
{
     ...
     * @var string
     */
    private $header = '<?xml version=\'1.0\' encoding=\'UTF-8\'?\\>' . PHP_EOL . '<Workbook xmlns=\'urn:schemas-microsoft-com:office:spreadsheet\'' . PHP_EOL . 'xmlns:x=\'urn:schemas-microsoft-com:office:excel\'' . PHP_EOL . 'xmlns:ss=\'urn:schemas-microsoft-com:office:spreadsheet\'' . PHP_EOL . 'xmlns:html=\'http://www.w3.org/TR/REC-html40\'>';
    /**
     * excel页脚
     * 依照excel xml规范。
     *
    ...
    private function addRow($array)
    {
        ...
        foreach ($array as $k => $v) {
            ...
            $cells .= '<Cell ' . $style_str . ' ><Data ss:Type=\'' . $format_str . '\'>' . $v['data'] . '</Data></Cell>' . PHP_EOL;
        }
        //构建行数据
        $this->lines[] = '<Row>' . PHP_EOL . $cells . '</Row>' . PHP_EOL;
    }
    /**
     * 添加多行数据
     * @access public
    ...
    public function addWorksheet($sheettitle)
    {
        ...
        $this->worksheets[] = PHP_EOL . '<Worksheet ss:Name=\'' . $sheettitle . '\'>' . PHP_EOL . '<Table ss:DefaultRowHeight=\'20\'>' . PHP_EOL . '<Column ss:Index=\'1\' ss:AutoFitWidth=\'0\'/>' . PHP_EOL . implode(PHP_EOL, $this->lines) . '</Table>' . PHP_EOL . '</Worksheet>' . PHP_EOL;
    }
    /**
     * 设置单元格样式
     * 
    ...
    public function generateXML($filename)
    {
        ...
        echo PHP_EOL . '</Styles>';
        //工作表
        echo implode(PHP_EOL, $this->worksheets);
        echo $this->footer;
    }
    /**
     * 转码函数
