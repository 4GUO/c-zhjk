...
class shop_spec extends control {
	...
	public function indexOp(){
		...
		$result = model('shop_goods_class')->getList(array('gc_parent_id' => 0, 'gc_state' => 1));
	...
	public function addOp(){		
		...
		if(chksubmit()){
			...
			$spec_info = $model_spec->getInfo(array('spec_name' => input('spec_name', ''), 'store_id' => array(0,$this->store_id)));
			...
			if($is_image == 1){
				$spec_info = $model_spec->getInfo(array('is_image' => 1, 'store_id' => array(0,$this->store_id)));
			...
			$spec_array = array();
			$spec_array['store_id'] = $this->store_id;           
	...
	public function editOp(){
		...
		if(chksubmit()){
			...
			if (empty($spec_info['store_id'])) {
				output_error('不可编辑平台规格！');
			}
			unset($spec_info);
			...
			$spec_array['is_image'] = $is_image;		
            $spec_array['spec_sort'] = input('spec_sort', 9999, 'intval');
            $state = $model_spec->edit(array('spec_id' => input('id', 0, 'intval'), 'store_id' => array(0,$this->store_id)), $spec_array);
			...
			$spec_info = $model_spec->getInfo(array('spec_id' => $spec_id, 'store_id' => array(0,$this->store_id)));
	...
	public function delOp(){
		...
		$spec_info = $model_spec->getInfo(array('spec_id' => $spec_id, 'store_id' => array(0,$this->store_id)));
		if (empty($spec_info['store_id'])) {
			output_error('不可删除平台规格！');
		}
		$where = array();
		$where['spec_id'] = $spec_id;
		$where['store_id'] = array(0,$this->store_id);
	...
	public function ajaxOp(){
		...
		$where = array(
			'store_id' => array(0,$this->store_id)
		);
		$gc_id = input('gc_id', 0, 'intval');
		if(empty($gc_id)){
			echo json_encode(array('state'=>400));
	...
	public function goods_specOp(){
        ...
        $goods_array = model('shop_goods')->getList(array('goods_commonid' => $common_id), 'goods_id,goods_marketprice,goods_price,goods_price_vip,goods_storage,goods_serial,goods_spec,goods_salenum');
        $sp_value = array();
