...
class shop_spec_value extends control {
	...
	public function addOp(){		
		...
		if(chksubmit()){
			...
			$sp_value_info = $model_spec_value->getInfo(array('sp_value_name' => input('sp_value_name', ''), 'store_id' => array(0,$this->store_id)));
	...
	public function editOp(){
		...
		if(chksubmit()){
			...
			if (empty($sp_value_info['store_id'])) {
				output_error('平台规格值不能被编辑！');
			}
			$spec_array = array();
			$spec_array['store_id'] = $this->store_id;
			$spec_array['sp_value_name'] = input('sp_value_name', '');         
	...
	public function delOp(){
		$model_spec_value = model('shop_spec_value');
		$sp_value_id = input('id', 0, 'intval');
		$sp_value_info = $model_spec_value->getInfo(array('sp_value_id' => $sp_value_id, 'store_id' => array(0,$this->store_id)));
		if (empty($sp_value_info['store_id'])) {
			output_error('平台规格值不能删除！');
		}
		$where = array();
