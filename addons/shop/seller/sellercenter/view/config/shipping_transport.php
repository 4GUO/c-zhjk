...
<div class='tabmenu'>
	<ul class='tab pngFix'>
		<li class='normal'><a href='<?=_url('config/shipping')?>'>综合设置</a></li>
		<li class='active'><a href='<?=_url('config/shipping_transport')?>'>区域运费</a></li>
	</ul>
	<a class='css-btn css-btn-green' href='<?=_url('config/shipping_transport_add')?>'>新增区域运费 </a> 
</div>
...
$(function(){	
	$('a.J_Delete').click(function(){
		...
		ajax_get_confirm('确认删除吗？', '<?=_url('config/shipping_transport_delete')?>?transport_id=' + id);
	});

	$('a.J_Modify').click(function(){
		var id = $(this).attr('data-id');
		if(typeof(id) == 'undefined') return false;
		$(this).attr('href', '<?=_url('config/shipping_transport_add')?>?transport_id=' + id);
		return true;
	});
