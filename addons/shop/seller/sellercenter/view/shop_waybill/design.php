...
.waybill_area {
	margin: 10px auto;
	width: <?php echo $output['waybill_info']['waybill_pixel_width'];?>px;
	height: <?php echo $output['waybill_info']['waybill_pixel_height'];?>px;
	position: relative;
	z-index: 1;
}
.waybill_back {
...
<div class='css-form-default'>
	<dl>
		<dt>选择打印项：</dt>
		<dd>
			<form id='design_form' action='<?=_url('shop_waybill/design')?>' method='post'>
				<input type='hidden' name='form_submit' value='ok' />
				<input type='hidden' name='waybill_id'
					value='<?php echo $output['waybill_info']['waybill_id'];?>'>
				<ul id='waybill_item_list' class='css-form-checkbox-list'>
					<?php if(!empty($output['waybill_item_list']) && is_array($output['waybill_item_list'])) {?>
					<?php foreach($output['waybill_item_list'] as $key => $value) {?>
					<li>
						<input id='check_<?php echo $key;?>' class='checkbox'
						type='checkbox' name='waybill_data[<?php echo $key;?>][check]'
						data-waybill-name='<?php echo $key;?>'
						data-waybill-text='<?php echo $value['item_text'];?>'
						<?php echo $value['check'];?>>
						<label
						for='check_<?php echo $key;?>' class='label'><?php echo $value['item_text'];?></label>
						
						<input
						id='left_<?php echo $key;?>' type='hidden'
						name='waybill_data[<?php echo $key;?>][left]'
						value='<?php echo $value['left'];?>'>
						<input
						id='top_<?php echo $key;?>' type='hidden'
						name='waybill_data[<?php echo $key;?>][top]'
						value='<?php echo $value['top'];?>'>
						<input
						id='width_<?php echo $key;?>' type='hidden'
						name='waybill_data[<?php echo $key;?>][width]'
						value='<?php echo $value['width'];?>'>
						<input
						id='height_<?php echo $key;?>' type='hidden'
						name='waybill_data[<?php echo $key;?>][height]'
	...
	<div>
		<div class='waybill_area'>
			<div class='waybill_back'> <img
					src='<?php echo $output['waybill_info']['waybill_image'];?>'
					alt=''> </div>
...
<script type='text/javascript' src='<?php echo STATIC_URL;?>/js/dialog/dialog.js' id='dialog_js' charset='utf-8'></script> 
