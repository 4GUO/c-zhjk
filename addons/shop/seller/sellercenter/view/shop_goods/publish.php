	...
	<form method='post' id='form' action='<?=users_url('shop_goods/publish')?>'>
		...
		<div class='css-form-goods'>
			...
			<dl>
				<dt>市场价：</dt>
				<dd>
					<input name='g_marketprice' value='<?=isset($output['goods']['goods_marketprice']) ? $output['goods']['goods_marketprice'] : ''?>' class='text w60' type='text'>
			...
			<dl>
				...
				<dd nc_type='no_spec'>
					<input name='g_freight' value='<?=$output['goods_freight']?>' class='text w60' type='text'>
					<em class='add-on'><i class='icon-renminbi'></i></em> <span></span>
			...
			<h3 id='demo2' style='display: none;'>商品视频</h3>
			<dl style='display: none;'>
				<dt>视频地址：</dt>
				<dd>
					<div style='overflow:hidden;'>
						<input nctype='goods_video' name='goods_video' class='text w400' value='<?=isset($output['upload_array']['goods_video']) ? $output['upload_array']['goods_video'] : (isset($output['goods']['goods_video']) ? $output['goods']['goods_video'] : '')?>' type='text' style='float:left;' />
						<span style='float:left;'>
							<div class='handle'>
								<a class='css-btn mt5' nc_type='dialog' dialog_title='上传文件' dialog_id='video' dialog_width='830' dialog_height='550' uri='<?=users_url('qiniu_file/upload_view', array('returnUrl' => base64_encode(users_url('shop_goods/publish', array('id' => isset($output['goods']['goods_commonid']) ? $output['goods']['goods_commonid'] : 0), false, true)), 'input_name' => 'goods_video'))?>' style='margin:0 10px;'><i class='icon-file'></i>上传视频</a>
								<a class='css-btn mt5' nc_type='dialog' dialog_title='已上传文件' dialog_id='video' dialog_width='830' dialog_height='550' uri='<?=users_url('qiniu_file/upload_list', array('returnUrl' => base64_encode(users_url('shop_goods/publish', array('id' => isset($output['goods']['goods_commonid']) ? $output['goods']['goods_commonid'] : 0), false, true)), 'input_name' => 'goods_video'))?>' style='margin:0 10px;'><i class='icon-link'></i>选择链接</a>
							</div>
						</span>
					</div>
					<p class='hint'>支持MP4格式，留空则不播放视频</p>
				</dd>
			</dl>
			<dl style='display: none;'>
				<dt>视频封面：</dt>
				<dd>
					<div class='css-goods-default-pic'>
						<div class='goodspic-uplaod'>
							<div class='upload-thumb'> <img nctype='goods_video_poster' src='<?=isset($output['goods']['goods_video_poster']) ? $output['goods']['goods_video_poster'] : STATIC_URL . '/images/default_image.png'?>'> </div>
							<input name='goods_video_poster' nctype='goods_video_poster' value='<?=isset($output['goods']['goods_video_poster']) ? $output['goods']['goods_video_poster'] : ''?>' type='hidden' />
							<span></span>
			...
			<dl>
				...
				<dd>
					<select name='stc_id'>
					    <?php foreach($output['store_class_list'] as $k => $v) {?>
						<?php if (!empty($v['child'])) {?>
						<option value='<?=$v['stc_id']?>' disabled='disabled'><?=$v['stc_name']?></option>
						<?php foreach($v['child'] as $c_k => $c_v){?>
						<option <?php if(isset($output['goods']['stc_id']) && $output['goods']['stc_id'] == $c_v['stc_id']){?> selected='selected'<?php }?> value='<?=$c_v['stc_id']?>'>&nbsp;&nbsp;└└ <?=$c_v['stc_name']?></option>
						<?php }?>
						<?php } else {?>
						<option <?php if(isset($output['goods']['stc_id']) && $output['goods']['stc_id'] == $v['stc_id']){?> selected='selected'<?php }?> value='<?=$v['stc_id']?>'><?=$v['stc_name']?></option>
						<?php }?>
						<?php }?>
					</select>
				</dd>
...
<script type='text/javascript' src='<?=STATIC_URL?>/admin/js/goods_add.js' charset='utf-8'></script>
<script>
