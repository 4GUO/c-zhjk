...
<div class='eject_con'>
	...
	<form id='category_form' method='post' target='_parent' action='<?=users_url('swiper/edit')?>'>
		...
		<dl>
			...
			<dd>
				<select name='module'>
					...
					<option value='<?php echo $module_id;?>'<?php echo $output['info']['module'] == $module_id ? ' selected' : '';?>><?php echo $module_name;?></option>
					<?php }?>
				</select>
			</dd>
		</dl>
			...
			<dd>
				<div class='css-goods-default-pic'>
					<div class='goodspic-uplaod'>
						<div class='upload-thumb'> <img nctype='image' src='<?=!empty($output['info']['image']) ? $output['info']['image'] : STATIC_URL . '/images/default_image.png'?>'> </div>
						<input name='image' nctype='image' value='<?=!empty($output['info']['image']) ? $output['info']['image'] : ''?>' type='hidden' />
						<span></span>
						<p class='hint'>建议使用<font color='red'>尺寸640*320像素、大小不超过1M的图片</font>，上传后的图片将会自动保存在图片空间的默认分类中。</p>
						<div class='handle'>
		...
		<dl style='display: none'>
			<dt>链接：</dt>
			<dd>
				<input class='text w200' type='text' name='href' id='href' value='<?=$output['info']['href']?>' />
			</dd>
		</dl>
		<dl>
