	...
	<div class='layui-row layui-col-space15'>
		<div class='flex-items todayboxs'>
			<a class='flex-item' href='javascript:;'>
				<img class='icon' src='<?=STATIC_URL?>/admin/images/Block-1.png'>
				<div class='text'>
					<div class='num'><?=$output['today_member_count']?></div>
			...
			<a class='flex-item' href='javascript:;'>
				...
				<div class='text'>
					<div class='num'><?=$output['today_tihuo_order_count']?></div>
					<div class='title'>今日提货订单</div>
				</div>
			</a>
			<a class='flex-item' href='javascript:;'> 
		...
		<div class='flex-items todayboxs'>
			<a class='flex-item' href='javascript:;'>
				<img class='icon' src='<?=STATIC_URL?>/admin/images/Block-2.png'>
				<div class='text'>
					<div class='num'><?=$output['use_tihuoquan_num']?></div>
			...
			<a class='flex-item' href='<?=users_url('index/untihuoquan_list')?>'>
				...
				<div class='text'>
					...
					<div class='title'>未提货提货券数量</div>
				</div>
			</a>
			<a class='flex-item' href='javascript:;'> 
				<img class='icon' src='<?=STATIC_URL?>/admin/images/Block-5.png'>
				<div class='text'>
					<div class='num'><?=$output['used_fenhong_quan']?></div>
					<div class='title'>已消耗分红券数量</div>
				</div>
			</a>
			<a class='flex-item' href='javascript:;'> 
				<img class='icon' src='<?=STATIC_URL?>/admin/images/Block-6.png'>
				<div class='text'>
					<div class='num'><?=$output['fenhong_quan']?></div>
					<div class='title'>待消耗分红券数量</div>
				</div>
			</a>
		</div>
		<div class='flex-items todayboxs'>
			<a class='flex-item' href='javascript:;'>
				<img class='icon' src='<?=STATIC_URL?>/admin/images/Block-3.png'>
				<div class='text'>
					<div class='num'><?=$output['today_yeji']?></div>
					<div class='title'>今日代理区营业额</div>
				</div>
			</a>
			<a class='flex-item' href='javascript:;'> 
				<img class='icon' src='<?=STATIC_URL?>/admin/images/Block-5.png'>
				<div class='text'>
					<div class='num'><?=$output['total_yeji']?></div>
					<div class='title'>代理区累计营业额</div>
				</div>
			</a>
		</div>
				...
				<div class='mypanel tasks-panel'>
					...
					<div class='mypanel-body'>
						<div class='flex-items tasks'>
							<div class='flex-item'> 
								<div>待发货</div>
								<a class='num state_pay' href='<?=_url('shop_order/index', array('state_type' => 'state_pay'))?>'></a>
							</div>
							
							<div class='flex-item'>
								<div>待退款</div>
								<a class='num hasnum state_refund' href='<?=_url('shop_return/index')?>'></a>
							</div>
						</div>
						<div class='flex-items tasks'>
							<div class='flex-item'>
								<div>会员提现</div>
								<a class='num hasnum w_new' href='<?=_url('withdraw_record/index', array('status' => 1))?>'></a>
							</div> 
							<div class='flex-item'>
								<div>待审核评价</div>
								<a class='num hasnum wait_shen_order_comment_count' href='<?=_url('shop_evaluate/index') ?>'></a>
							</div>
						</div>
						<div class='flex-items tasks'>
							<div class='flex-item'>
								<div>待审核商品</div>
								<a class='num hasnum wait_shen_goods_count' href='<?php echo _url('shop_goods/index', array('goods_state' => 1)); ?>'></a>
							</div> 
							<div class='flex-item'>
								<div>商家提现</div>
								<a class='num store_tixian_count' href='<?php echo _url('store/tixianlist'); ?>'></a>
							</div>
						</div>
					</div>
				</div>
		...
		<div class='row-panel'>
			<div class='row-panel-7'>
				<div class='mypanel total-panel'>
					<div class='mypanel-heading'>本月商家销售排行</div>
					<div class='mypanel-body'>
						<div id='echat_month_store_sales' style='height:460px;'></div>
					</div>
				</div>
			</div>
			<div class='row-panel-5'>
				<div class='mypanel total-panel'>
					<div class='mypanel-heading'>上月商家销售排行</div>
					<div class='mypanel-body'>
						<div id='echat_lastmonth_store_sales' style='height:460px;'></div>
					</div>
				</div>
			</div>
		</div>
...
$(function(){
	...
	getAjax('<?=_url('index/withdraw_record')?>?rand=' + timestamp, {}, function(e) {
        if (e.state == 400) return false;
		var data = e.data;
        for(var a in data) {
            if(data[a] != 'undefined') {
	...
	loadEchartsLine();
	load_echat_month_store_sales();
	load_echat_lastmonth_store_sales();
...
function load_echat_month_store_sales() {
	var hasLineChart = $('#echat_month_store_sales').length>0;
	if (hasLineChart) {
		var lineChart = echarts.init(document.getElementById('echat_month_store_sales'));
	}
	window.onresize = function () {
		if(hasLineChart) {
			lineChart.resize();
	...
	$.ajax({
		type: 'GET',
		url: '<?=_url('index/load_echat_month_store_sales', array('type' => 1))?>',
		dataType: 'json',
		success: function (data) {
			if(data.code == 0) {
				...
				for( var i in data.list ) {
					le_arr.push(  data.list[i].storename  );
					data_obj_arr.push({value:data.list[i].total, name:data.list[i].storename });
				}
...
function load_echat_lastmonth_store_sales() {
	var hasLineChart = $('#echat_lastmonth_store_sales').length > 0;
	if (hasLineChart) {
		var lineChart = echarts.init(document.getElementById('echat_lastmonth_store_sales'));
	}
	window.onresize = function () {
		if(hasLineChart) {
			lineChart.resize();
	...
	$.ajax({
		type: 'GET',
		url: '<?=_url('index/load_echat_month_store_sales', array('type' => 2))?>',
		dataType: 'json',
		success: function (data) {
			if(data.code == 0) {
...
function load_echat_member_incr() {
	...
	$.ajax({
		type: 'GET',
		url: '<?=_url('index/load_echat_member_incr')?>',
		dataType: 'json',
		success: function (data) {
			if(data.code == 0) {
				var option = null;
