...
<form method='get' action='<?=users_url('shop_vr_return/index')?>'>
	...
	<tbody>
		...
		<tr class='bd-line'>
			...
			<td class='bdl bdr' id='btns_<?php echo $val['refund_id']; ?>'>
				<p><a href='<?=users_url('shop_vr_order/show_order', array('order_id' => $val['order_id']))?>' class='css-btn css-btn-orange mt5'><i class='icon-truck'></i>相关订单</a></p> 
				...
				<p><a href='javascript:void(0)' class='css-btn css-btn-green mt5 delete' confirm='确定要退款?' url='<?=users_url('shop_vr_return/pay_record', array('refund_id' => $val['refund_id'], 'refund_type' => 'offline'))?>' /><i class='icon-pencil'></i>线下退款</a></p>
                <p><a href='javascript:void(0)' class='css-btn css-btn-red mt5' nc_type='dialog' uri='<?=users_url('shop_vr_return/reject', array('refund_id' => $val['refund_id']))?>' dialog_title='退款申请驳回' dialog_id='cancel_reocrd' dialog_width='400' id='reocrd<?php echo $val['refund_id']; ?>' /><i class='icon-remove-circle'></i>驳回申请</a></p>
                <?php } ?>				
			</td>
		</tr>
		<?php } ?>		
...
$(function(){
	$('a[nctype=pay_record]').on('click', '', function(){
		...
		$('#btns_' + rid).html('');
		getAjax('<?=users_url('shop_vr_return/pay_record')?>', {refund_id: rid}, function(e){
			if(e.state == 400){
				showError(e.msg, function(){window.location.reload();}, 'center', '', '', '', '');
			} else {
				showSucc(e.msg, function(){window.location.reload();}, 'center', '', '', '', '');
			}			
		});
	});
});
