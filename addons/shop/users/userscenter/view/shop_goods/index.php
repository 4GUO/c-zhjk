...
<div class='tabmenu'>
	<ul class='tab pngFix'>
		<li class='<?php if (input('goods_state', 2) == 2) { ?>active<?php } else { ?>normal<?php } ?>'><a href='<?=users_url('shop_goods/index', array('goods_state' => 2))?>'>出售商品</a></li>
		<li class='<?php if (input('goods_state', 2) == 1) { ?>active<?php } else { ?>normal<?php } ?>'><a href='<?=users_url('shop_goods/index', array('goods_state' => 1))?>'>下架商品</a></li>
	</ul>
	...
	<tbody>
		...
		<tr>
					...
					<dd class='serve'> 
						...
						<span class='' title='商品手机端连接'>
							...
							<div style='display:none;'>
								<?=front_url('goods/goods_info', array(), false, true)?>?i=<?=$output['setting']['uniacid']?>&goods_id=<?=$val['goods_id']?>
							</div>
						</span>
					</dd>
				</dl>
			</td>
			<td><?php echo empty($output['store_name_arr'][$val['store_id']]) ? '暂无' : $output['store_name_arr'][$val['store_id']]?></td>
			<td><?php echo $val['is_experience_goods'] ? '是' : '不是'?></td>
			...
			<td class='nscs-table-handle'>
				...
				<span>
					<a href='javascript:;' confirm='您确定下架此商品?' url='<?=users_url('shop_goods/uncheck', array('commonid' => $val['goods_commonid']))?>' class='btn-blue delete'><i class='icon-edit'></i>
						<p>下架</p>
					</a>
				</span>
				<?php } ?>
				<?php if ($val['goods_state'] == 0) { ?>
				<span>
					<a href='javascript:;' confirm='您确定上架此商品?' url='<?=users_url('shop_goods/check', array('commonid' => $val['goods_commonid']))?>' class='btn-blue delete'><i class='icon-edit'></i>
						<p>上架</p>
					</a>
				</span>
				<span>
