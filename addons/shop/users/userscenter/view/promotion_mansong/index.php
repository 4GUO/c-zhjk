	...
	<tbody id='mansong_list'>
		<tr class='bd-line'>
			...
			<td class='nscs-table-handle tc'>
				<span> 
					<a href='<?=users_url('promotion_mansong/info', array('mansong_id' => $val['mansong_id']))?>' class='btn-blue'>
						<p>详情</p>
					</a> 
				</span>
				<?php if ($val['state'] != 3) { ?>
				<span> 
					<a href='javascript:void(0)' confirm='确定取消活动?' url='<?=users_url('promotion_mansong/cancel', array('mansong_id' => $val['mansong_id']))?>' class='btn-blue delete'>
						<p>取消</p>
					</a> 
				</span>
				<?php } ?>
				<span> 
					<a href='javascript:void(0)' confirm='确定删除?' url='<?=users_url('promotion_mansong/del', array('mansong_id' => $val['mansong_id']))?>' class='btn-red delete'>
						<p>删除</p>
					</a> 
				</span>
			</td>
