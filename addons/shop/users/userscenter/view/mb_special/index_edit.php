		...
		<div id='item_list' class='item-list'>
			...
			<div nctype='special_item' class='special-item <?php echo $value['item_type'];?> <?php echo $value['usable_class'];?>' data-item-id='<?php echo $value['item_id'];?>'>
				...
				<div id='item_edit_content'>
					<?php require('module_' . $value['item_type'] . '.php');?>
				</div>
...
<!-- 页面模块模板 --> 
<script id='item_template' type='text/html'></script>
<script type='text/javascript'>
    var special_id = <?php echo $output['special_id'];?>;
    var url_item_add = '<?php echo _url('mb_special/special_item_add');?>';
    var url_item_del = '<?php echo _url('mb_special/special_item_del');?>';
    var url_item_edit = '<?php echo _url('mb_special/special_item_edit');?>';
    $(document).ready(function(){
        ...
        $('#item_list').on('click', '[nctype=btn_del_item]', function() {
			...
			ajax_get_confirm('确认删除？', url_item_del + '?item_id=' + item_id + '&special_id=' + special_id, function(data) {
				if(typeof data.error === 'undefined') {
        ...
        $('#item_list').on('click', '[nctype=btn_edit_item]', function() {
            var item_id = $(this).attr('data-item-id');
			window.location = url_item_edit + '?item_id=' + item_id;
        });
        ...
        var update_item_sort = function() {
            ...
            $.post('<?=_url('mb_special/update_item_sort')?>', {special_id: special_id, item_id_string: item_id_string}, function(data) {
                if (typeof data.error != 'undefined') {
        ...
        $('#item_list').on('click', '[nctype=btn_usable]', function() {
            ...
            $.post('<?=_url('mb_special/update_item_usable')?>', {item_id: item_id, usable: usable, special_id: special_id}, function(data) {
                if (typeof data.error != 'undefined') {
                    showError(data.message);
                }
            }, 'json');
