...
<div class='tabmenu'>
	<ul class='tab pngFix'>
		<li class='<?=empty(input('state_type', '')) ? 'active' : 'normal'?>'>
			<a href='<?=users_url('tihuo_order/index')?>'>所有订单</a>
		</li>
		<!--<li class='<?=input('state_type', '') == 'state_new' ? 'active' : 'normal'?>'>
			<a href='<?=users_url('tihuo_order/index', array('state_type' => 'state_new'))?>'>待付款</a>
		</li>-->
		<li class='<?=input('state_type', '') == 'state_pay' ? 'active' : 'normal'?>'>
			<a href='<?=users_url('tihuo_order/index', array('state_type' => 'state_pay'))?>'>待发货</a>
		</li>
		<li class='<?=input('state_type', '') == 'state_send' ? 'active' : 'normal'?>'>
			<a href='<?=users_url('tihuo_order/index', array('state_type' => 'state_send'))?>'>已发货</a>
		</li>
		<li class='<?=input('state_type', '') == 'state_success' ? 'active' : 'normal'?>'>
			<a href='<?=users_url('tihuo_order/index', array('state_type' => 'state_success'))?>'>已完成</a>
		</li>
		<li class='<?=input('state_type', '') == 'state_cancel' ? 'active' : 'normal'?>'>
			<a href='<?=users_url('tihuo_order/index', array('state_type' => 'state_cancel'))?>'>已取消</a>
		</li>
	</ul>
</div>
<form method='get' action='<?=users_url('tihuo_order/index', array('state_type' => input('state_type')))?>'>
...
<table class='css-default-table order' id='my_agent'>
	...
	<tbody>
		...
		<tr>
			...
			<span class='fr mr5'> <a href='<?=users_url('tihuo_order/print_order', array('order_id' => $val['order_id']))?>' class='css-btn-mini' target='_blank' title='打印发货单'/><i class='icon-print'></i>打印发货单</a></span>
			</th>
		</tr>
		<?php $i = 0;?>
		...
		<tr>
			...
			<td class='bdl' rowspan='<?php echo $val['goods_count'];?>'><div class='buyer'><?php echo $val['member_name'];?>					
			...
			<td class='bdl bdr' rowspan='<?php echo $val['goods_count'];?>'><p><?php echo $val['state_desc']; ?></p>
				<!-- 订单查看 -->
				<p><a href='<?=users_url('tihuo_order/show_order', array('order_id' => $val['order_id']))?>'>订单详情</a></p>
...
$('#export').click(function() {
	var url = '<?=users_url('tihuo_order/index', array('state_type' => input('state_type')))?>?<?php echo $_SERVER['QUERY_STRING'];?>&is_export=1';
