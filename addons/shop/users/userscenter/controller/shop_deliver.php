...
class shop_deliver extends control {
    ...
    public function sendOp()
    {
        ...
        if (chksubmit()) {
            ...
            $result = $model_order->changeOrderSend($order_info, 'admin', input('session.username', ''), $post);
            if (!$result['state']) {
			...
			if ($order_info['extend_order_common']['daddress_id'] > 0) {
				...
				//取默认地址
				$daddress_info = $model_daddress->getInfo(array('store_id' => 0), '*', 'is_default desc');
    ...
    public function send_address_selectOp()
    {
		...
		if (chksubmit()) {
			...
			$list_temp = model('shop_daddress')->getList(array('store_id' => 0), '*', 'address_id desc');
    ...
    public function waybill_printOp()
    {
        ...
        $waybill_info = $model_waybill->getInfo(array('store_id' => 0, 'waybill_usable' => 1, 'express_id' => $express_id));
