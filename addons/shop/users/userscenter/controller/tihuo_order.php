...
use lib;
class tihuo_order extends control {
	...
	public function indexOp(){
		...
		$this->assign('page', page(isset($list['totalpage']) ? $list['totalpage'] : 0, array('page' => input('get.page', 1, 'intval'), 'state_type' => $state_type, 'buyer_name' => $buyer_name, 'order_sn' => $order_sn, 'query_start_date' => $query_start_date, 'query_end_date' => $query_end_date), users_url('tihuo_order/index')));
		//物流
		$express_list = array();
    ...
    public function show_orderOp(){
		...
		if ($order_id <= 0) {
			...
			if ($order_sn) {
				...
				web_error('参数有误！', users_url('tihuo_order/index'));
			}
        }
        $condition = array();
		...
		if (empty($order_info['order_id'])) {
            web_error('订单不存在！', users_url('tihuo_order/index'));
        }
    ...
    public function order_cancelOp(){
		...
		if (chksubmit()) {
			...
			output_data(array('msg' => '取消成功', 'url' => users_url('tihuo_order/index')));
		} else {
			$this->assign('order_info', $order_info);
    ...
    public function shipping_priceOp(){
        ...
        if (chksubmit()) {
			...
			output_data(array('msg' => '修改成功', 'url' => users_url('tihuo_order/index')));
        } else {
            $this->assign('order_info', $order_info);
			$this->view->_layout_file = 'null_layout';
			$this->display();
    ...
    public function receive_payOp() {
		...
		if (chksubmit()) {
			...
			output_data(array('msg' => '操作成功', 'url' => users_url('tihuo_order/index')));
		} else {
			$order_id = input('order_id', 0, 'intval');
			$order_info = $model_order->getInfo(array('order_id' => $order_id));
