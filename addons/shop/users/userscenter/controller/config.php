...
class config extends control
{
    ...
    public function distributeOp()
    {
        if (chksubmit()) {
            ...
            $yeji_fenhong_level_ids_str = input('yeji_fenhong_level_ids', '')[0] ?? '';
            ...
            $data = array(
                ...
                'yeji_fenhong_level_ids' => $yeji_fenhong_level_ids_str,
        ...
        //会员级别
        $level_temp = model('vip_level')->getList(array('uniacid' => $this->uniacid), '*', 'level_sort ASC');
        ...
        if (!empty($this->setting_config['yeji_fenhong_level_ids'])) {
            ...
            $selected_level_ids = array_filter(array_map('intval', $selected_level_ids));
        }
