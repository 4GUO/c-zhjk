...
class shop_buy extends member
{
    ...
    public function buy_step1Op()
    {
		if(IS_API){
			...
			$logic_buy = logic('shop_buy');
			// 得到购买数据
			$result = $logic_buy->buyStep1($cart_id, $this->member_info, $city_id);
			if (!$result['state']) {
			...
			$buy_list['order_num'] = $result['final_order_num'];
			$buy_list['store_premiums_list'] = $result['store_premiums_list'];
			$buy_list['store_mansong_rule_list'] = $result['store_mansong_rule_list'];
			$buy_list['voucher_list'] = $result['voucher_list'];
			$buy_list['available_predeposit'] = $this->member_info['available_predeposit'];
			$buy_list['promotion_total'] = $result['promotion_total'];
			output_data($buy_list);
		}
    }
	public function change_voucherOp() {
		$cart_id = explode(',', input('cart_id', ''));
		$post_voucher = explode(',', input('voucher_info', ''));
		$voucher_info = array();
		...
		$logic_buy = logic('shop_buy');
		// 得到购买数据
		$result = $logic_buy->change_voucher($cart_id, $this->member_info, $voucher_info);
		if (!$result['state']) {
			output_error($result['msg']);
		} else {
			$result = $result['data'];
    ...
    {
		if (IS_API) {
		    $reporter_id = intval(trim(input('reporter_id', '', 'trim'), config('uid_pre')));
		    if ($reporter_id) {
		        $reporter_member = model('member')->where(array('uid' => $reporter_id))->find();
		        if (empty($reporter_member['can_baodan'])) {
		            output_error('编号未授权，不能下单');
		        }
		    }
			$param = array();
			...
			$param['reporter_uid'] = $reporter_id;
    ...
    {
		// 取订单信息
        $result = logic('shop_buy')->getRealOrderInfo($pay_sn, $this->member_info['uid']);
        if (!$result['state']) {
        ...
        if ($pay_amount <= 0) {
			if($this->client_type == 'wxapp') {
				output_data(array('msg' => '支付成功', 'url' => '/pages/order/index'));
    ...
    private function _pd_pay($order_list, $post)
    {
        ...
        try {
			...
			if ($update_pay_order_list) {
				$result = logic('shop_buy')->updateOrderAll($update_pay_order_list, 'predeposit');
				if (!$result['state']) {
