...
class vr_order extends member {
		...
		if(IS_API){
			...
			if(!empty($list['list'])){
				foreach ($list['list'] as $value) {
					...
					//显示退款按钮
					$value['if_refund_cancel'] = $model_order->getOrderOperateState('refund', $value);
					//显示删除按钮
					...
					$group_order_list[$value['order_sn']]['order_list'][] = $value;
					//如果有在线支付且未付款的订单则显示合并付款链接
					if ($value['order_state'] == ORDER_STATE_NEW) {
						if (!isset($group_order_list[$value['order_sn']]['pay_amount'])) {
							$group_order_list[$value['order_sn']]['pay_amount'] = 0;
						}
						$group_order_list[$value['order_sn']]['pay_amount'] += $value['order_amount'] - $value['pd_amount'];
					}
					$group_order_list[$value['order_sn']]['add_time'] = $value['add_time'];
					$group_order_list[$value['order_sn']]['order_sn'] = $value['order_sn'];
				}
			}
			$return = array(
				'title' => $this->title,
	...
	public function order_infoOp() {
		...
		if (IS_API) {
			...
			$order_info = $model_order->getInfo($where);
			if(!$order_info){
				output_error('订单不存在');
			}
			//显示取消订单
	...
	public function s_up_img_updateOp(){
		if (IS_API) {
			...
			if (!empty($_FILES[$file_name]['name'])) {
				...
				$upload->set('fprefix', 'vr_' . $order_id);
	...
	public function refund_pageOp() {
		if (IS_API) {
			...
			$condition['buyer_id'] = $this->member_info['uid'];
			$order_info = $model_order->getInfo($condition);
			$if_evaluation = $model_order->getOrderOperateState('refund', $order_info);
			if (!$if_evaluation) {
				output_error('退款失败');
			}
	...
	public function refund_submitOp(){
		if (IS_API) {
			...
			$shop_refund_return = model('shop_vr_refund_return');
			$refund_array = array();
			...
			$refund_array['buyer_id'] = $order_info['buyer_id'];
			$refund_array['buyer_name'] = $order_info['buyer_name'];
			$refund_array['refund_amount'] = floatval($order_info['order_amount']);
			...
			if($refund_id){
				$model_order->edit(array('order_id' => $order_id), array('refund_state' => 2));
				output_data('1');
			}
		}
	}
	public function refund_listOp(){
		if(IS_API){
			$this->title = '退款记录';
			$model_refund = model('shop_vr_refund_return');
			$where = array();
			$where['buyer_id'] = $this->member_info['uid'];
