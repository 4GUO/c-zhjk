...
class shop_cart extends member
{
    ...
    public function cart_addOp() {
		if(IS_API){
			...
			/*$result = logic('shop_buy')->check_buy(0, $cart_info['goods_num'] + $quantity, $goods_info);
			if (!$result['state']) {
    ...
    public function cart_edit_quantityOp()
    {
		if(IS_API){
			...
			// 检查库存是否充足
			/*$result = logic('shop_buy')->check_buy(0, $quantity, $goods_info);
			if (!$result['state']) {
				output_error($result['msg']);
			}*/
			unset($result);
	...
	public function cart_selectOp() {
		if(IS_API){
			$model_cart = model('shop_cart');
			$cart_id = input('cart_id', 0, 'intval');
			$selected = input('selected', 0, 'intval');
			$data = array();
	...
	public function cart_selectAllOp() {
		if(IS_API){
			$model_cart = model('shop_cart');
			$selected = input('selectedAll', 0, 'intval');
			$data = array();
			$data['selected'] = $selected;
			$update = $model_cart->edit($data, array('uid' => $this->member_info['uid']));
			$cart_array = $this->get_cart_list();
			output_data($cart_array);
		}
	}
	private function get_cart_list(){
		$model_cart = model('shop_cart');
		...
		sort($cart_a);
        return array('cart_list' => $cart_a, 'total_money' => priceFormat($total_money), 'selectedAll' => $selectedAll, 'cart_count' => count($cart_list_real));
	}
}